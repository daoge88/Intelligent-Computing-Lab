...
class AttModel(nn.Module):
    def __init__(self, hp_, enc_voc, dec_voc):
        ...
        for i in range(self.hp.num_blocks):
            self.__setattr__('enc_self_attention_%d' % i,
                             multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads,
                                                 self.hp.dropout_rate, causality=False))
            self.__setattr__('enc_feed_forward_%d' % i,
                             feedforward(self.hp.hidden_units,
        ...
        for i in range(self.hp.num_blocks):
            self.__setattr__('dec_self_attention_%d' % i,
                             multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads,
                                                 self.hp.dropout_rate, causality=True))
            self.__setattr__('dec_vanilla_attention_%d' % i,
                             multihead_attention(self.hp, self.hp.hidden_units, self.hp.num_heads,
                                                 self.hp.dropout_rate, causality=False))
            self.__setattr__('dec_feed_forward_%d' % i,
                             feedforward(self.hp.hidden_units,
                                         [4 * self.hp.hidden_units, self.hp.hidden_units]))
    ...
    def forward(self, x, y):
        ...
        for i in range(self.hp.num_blocks):
            ...
            # vanilla (cross) attention: queries from decoder, keys/values from encoder
